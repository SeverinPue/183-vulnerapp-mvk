    ...
    public Customizer<FormLoginConfigurer<HttpSecurity>> restfulFormLogin() {
        return form -> form.failureHandler((req, res, ex) -> res.sendError(HttpServletResponse.SC_FORBIDDEN, ex.getMessage()))
                .successHandler((request, response, auth) -> {
                    ...
                    var user = userRepository.findById(auth.getName()).orElseThrow();
                    var userResponse = buildUserResponse(user);
                    response.getWriter().write(objectMapper.writeValueAsString(userResponse));
                });
    }

    private Map<String, Object> buildUserResponse(ch.bbw.m183.vulnerapp.datamodel.UserEntity user) {
        return Map.ofEntries(
                Map.entry("username", user.getUsername()),
                Map.entry("email", user.getEmail()),
                Map.entry("fullname", user.getFullname()),
                Map.entry("enabled", user.isEnabled()),
                Map.entry("roles", user.getRoles().stream().map(ch.bbw.m183.vulnerapp.datamodel.RoleEntity::getName).toList())
        );
    }
